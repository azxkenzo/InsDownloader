...
class LoginActivity : AppCompatActivity() {
    ...
    private class LoginWebViewClient(private val context: Context) : WebViewClient() {
        ...
        override fun shouldInterceptRequest(
            ...
            println(request?.url) // /graphql/query/
            ...
            request?.url?.let {
                if (it.path == "/graphql/query/" && !TextUtils.isEmpty(it.query)) {
                    val queryHashPatt =
                        Pattern.compile("query_hash=(.{28,36})&vari.{30,50}\"after\":")
        ...
        override fun onPageFinished(view: WebView?, url: String?) {
            url?.let {
                println("LoginActivity.onPageFinished  in  ${Thread.currentThread().name}")
                ...
                cookie?.let {
                    ...
                    val csrftokenPatt = Pattern.compile("csrftoken=(.+?);")
                    ...
                    val sessionidPatt = Pattern.compile("sessionid=(.+?);")
                    ...
                    val dsUserIdPatt = Pattern.compile("ds_user_id=(.+?);")
                    val dsUserIdMatcher = dsUserIdPatt.matcher(cookie)
                    if (dsUserIdMatcher.find()) {
                        Downloader.dsUserId = dsUserIdMatcher.group(1) ?: ""
                        context.getSharedPreferences("config", Context.MODE_PRIVATE).edit {
                            putString("dsUserId", dsUserIdMatcher.group(1))
                            commit()
                        }
                    }
                }
